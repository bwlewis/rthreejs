...
The new `grapjhs` function (an html widget), part of the threejs package, makes it easy to plot
...
by Donaled Knuth(http://www-cs-faculty.stanford.edu/~uno/sgb.html) and taken from
the D3.js force directed graph example by Mike Bostock (http://bl.ocks.org/mbostock/4062045).

We explore the `graphhs` function by looking at the graph of character coappearance
...
library(threejs)
data(LeMis)
graphjs(LeMis$nodes, LeMis$edges, bg="#fbfbfb")
```
...
is, nodes connected to other nodes that are also connected receive more weight.
...
(see, for example, E. Estrada and D. J. Higham, Network properties revealed through matrix functions, SIAM Rev., 52 (2010), pp. 696–714.) adjust this concept to
